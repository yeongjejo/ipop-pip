...
class PIP(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward_frame(self, glb_acc, glb_rot, check_rbdl, position, return_grf=False):
        ...
        return self.dynamics_optimizer.optimize_frame_1(pose, joint_velocity, [], glb_acc.cpu(), check_rbdl, position=position,
                                                     return_grf=return_grf)
        # return self.dynamics_optimizer.optimize_frame(pose, joint_velocity, [], glb_acc.cpu(), check_rbdl,
        #                                                return_grf=return_grf)
