...
from slime_reset.new_reset import NewReset
from slime_reset.quaternion_mobing_average import QuaternionMovingAverage
...
class UDPServer(threading.Thread):
    ...
    def run(self):
        ...
        axes_offset = Quaternion().from_rotation_vector(Vector3(-(0.5 * math.pi), 0.0, 0.0))
        ...
        while self._running:
            ...
            while True:
              ...
              # 쿼터니언 w, x, y, z 계산
                ...
                # 슬라임VR 리셋 부분
                # quaternion = axes_offset.multiply(quaternion)
                # moving_avg = QuaternionMovingAverage()
                # new_reset = NewReset()
                # if sensor_part in DataManager().sensor_data:
                #     moving_avg = DataManager().sensor_data[sensor_part][4]
                #     new_reset = DataManager().sensor_data[sensor_part][5]

                # moving_avg.get_fps_timer().update() #fps 업데이트
                # moving_avg.update()
                # moving_avg.add_quaternion(quaternion)
                
                # # 1, 2차 필터
                # filter_quaternion = moving_avg.get_filtered_quaternion() #1차 필터
                # filter_quaternion = new_reset.adjust_to_reference(filter_quaternion) #2차 필터
                
                
                # filter_quaternion.set_raw_data(quaternion)
                
                # # 센서 정보 저장
                # DataManager().sensor_data = [sensor_part, [gyro, acc, mag, filter_quaternion, moving_avg, new_reset]]
